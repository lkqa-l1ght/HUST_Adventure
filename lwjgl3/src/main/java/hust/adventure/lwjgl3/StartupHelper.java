...
/**
 * A helper object for game startup, featuring three utilities related to LWJGL3
 * on various operating systems.
 * <p>
 * The utilities are as follows:
 * <ul>
 * <li>Windows: Prevents a common crash related to LWJGL3's extraction of shared
 * library files.</li>
 * <li>macOS: Spawns a child JVM process with {@code -XstartOnFirstThread} in
 * the JVM args (if it was not already).
 * This is required for LWJGL3 to work on macOS.</li>
 * <li>Linux (NVIDIA GPUs only): Spawns a child JVM process with the
 * {@code __GL_THREADED_OPTIMIZATIONS}
 * {@link System#getenv(String) Environment Variable} set to {@code 0} (if it
 * was not already). This is required for
...
public class StartupHelper {
	...
	/**
	 * Must only be called on Linux. Check OS first (or use short-circuit
	 * evaluation)!
	 * 
	 ...
	 * <p>
	 * All {@link System#getenv() Environment Variables} are copied to the child JVM
	 * process (if it is spawned), as
	 ...
	 *                  block the thread until the child JVM process stops
	 *                  executing.
	 * @return whether a child JVM process was spawned or not.
	 */
	 ...
	 * {@link System#getProperties() System Properties}.
	 * 
	 * @param isMac     whether the current OS is macOS. If this is `false` then the
	 *                  current OS is assumed to be Linux (and
	...
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		...
		// This state shouldn't usually be reachable, but this stops us from endlessly
		// spawning new child JVM processes.
